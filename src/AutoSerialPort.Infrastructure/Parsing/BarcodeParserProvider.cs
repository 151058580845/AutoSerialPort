...
/// 扫码枪解析器提供者。
/// </summary>
public class BarcodeParserProvider : IParserProvider
{
    /// <summary>
    ...
    /// 解析器类型标识。
    /// </summary>
    public string Type => "BarcodeParser";

    /// <summary>
    /// 界面展示名称。
    /// </summary>
    public string DisplayName => "扫码枪解析器";

    /// <summary>
    /// 默认参数 JSON。
    /// </summary>
    public string DefaultParametersJson => JsonSerializer.Serialize(new BarcodeParserOptions(), JsonOptions);
    ...
    public IParser Create(string? parametersJson)
    {
        ...
        return new BarcodeParser(options);
    }
